...
You can preview the default invoice output generated by the package here:

- [invoice-sample.pdf](../assets/invoice-sample.pdf)

This PDF is generated with the default Blade views shipped by `proovit/laravel-billing`.
The web preview and print views reuse the same shared invoice document partial, so the rendered invoice stays consistent across channels.
